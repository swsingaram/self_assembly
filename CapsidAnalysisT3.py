...
import numpy as np
BOX = 63 #cubic box with length 40
TRAJFILE = './temp.mol2'
FRAMEA = 1
FRAMEB = 70 #150
NUMCP = 270 #158
...
QCHARGE = float(sys.argv[1])
POLLENGTH = float(sys.argv[2]) #800
EBOND = float(sys.argv[3])
ITR = int(sys.argv[4])
numboundfile = 'numbound_%s_%s_%s_%s.dat' % (QCHARGE, POLLENGTH, EBOND,ITR)
clusterfile = 'cluster_%s_%s_%s_%s.dat' % (QCHARGE, POLLENGTH, EBOND,ITR)
######
CUTOFFCPRNA = 3.0
CUTOFFCPCP  = 4*0.1
